...
smooth(unsigned int _iters)
{
    ...
    for (unsigned int iter=0; iter<_iters; ++iter)
    {
        ...
        QualityViewer::calc_edge_weights();
        ...
        for (v_it=mesh_.vertices_begin(); v_it!=v_end; ++v_it)
        {
            ...
            if(iter == 0)
            {
                v_old_pos_[*v_it] = mesh_.position(*v_it);
            }
            ...
            if (!mesh_.is_boundary(*v_it))
            {
                ...
                v_new_pos_[*v_it] = mesh_.position(*v_it) + 0.5 * laplace;
        ...
        for(v_it = mesh_.vertices_begin(); v_it != v_end; ++v_it)
        {
            if (!mesh_.is_boundary(*v_it))
            {
                mesh_.position(*v_it) = v_new_pos_[*v_it];
            }
        }
    }
	...
	for (unsigned int iter=0; iter<_iters; ++iter)
	{
        ...
        for(v_it = mesh_.vertices_begin(); v_it != v_end; ++v_it)
        {
            ...
            if(!mesh_.is_boundary(*v_it))
            {
                ...
                do{
                    laplacian += (mesh_.position(*vv_c) - mesh_.position(*v_it));
                    ++n;
                }while(++vv_c != vv_end);

                laplacian /= n;
                v_new_pos_[*v_it] = mesh_.position(*v_it) + 0.5 * laplacian;
            }
        }
...
uniform_laplacian_enhance_feature()
{
    ...
    for(v_it = mesh_.vertices_begin(); v_it != v_end; ++v_it)
    {
        if(!mesh_.is_boundary(*v_it))
        {
            mesh_.position(*v_it) = v_new_pos_[*v_it] + enhancement_coef_ * (v_old_pos_[*v_it] - v_new_pos_[*v_it]);
        }
    }
