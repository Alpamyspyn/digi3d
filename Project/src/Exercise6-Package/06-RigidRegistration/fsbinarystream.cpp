...
fsMsgPtr fsBinaryStream::get_message() {
    ...
    case fsMsg::MSG_OUT_MARKER_NAMES: {
        QSharedPointer< fsMsgMarkerNames > msg(new fsMsgMarkerNames());
        ...
        if( actual_size != super_block.size ) { LOG_RELEASE_ERROR("Block was promised to be of size %d, not %lld", super_block.size, actual_size); m_valid = false; return fsMsgPtr(); }
        return msg;
    }; break;
    case fsMsg::MSG_OUT_BLENDSHAPE_NAMES: {
        QSharedPointer< fsMsgBlendshapeNames > msg(new fsMsgBlendshapeNames() );
    ...
    case fsMsg::MSG_OUT_TRACKING_STATE: {
        ...
        QSharedPointer<fsMsgTrackingState> msg = QSharedPointer<fsMsgTrackingState>(new fsMsgTrackingState());
        for(int i = 0; i < num_blocks; i++) {
            ...
            if( actual_size != sub_block.size ) {
                LOG_RELEASE_ERROR("Unexpected number of bytes consumed %lld instead of %d for subblock %d id:%d", actual_size, sub_block.size, i, sub_block.id);
                m_valid = false; return fsMsgPtr();
            }
        }
        uint64_t actual_size =  m_start-super_block_data_start;
        if( actual_size != super_block.size ) {
            LOG_RELEASE_ERROR("Unexpected number of bytes consumed %lld instead of %d", actual_size, super_block.size);
            m_valid = false; return fsMsgPtr();
        }
        return msg;
    }; break;
    case fsMsg::MSG_OUT_RIG: {
        QSharedPointer< fsMsgRig > msg(new fsMsgRig() );
